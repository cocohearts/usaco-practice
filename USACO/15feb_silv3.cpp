...
void solve() {
	...
	priority_queue<pair<int,pi>> pairings;
	F0R(i,N) {
		FOR(j,i+1,N) {
			pairings.push(mp(ids[i]^ids[j],mp(i,j)));
		}
	}
	DSU myDSU(N);
	ll score = 0;
	while (myDSU.size(0)!=N) {
		auto np = pairings.top(); pairings.pop();
		if (myDSU.join(np.s.f,np.s.s)) {
			score += np.f;
		}
	}
	cout << score;
